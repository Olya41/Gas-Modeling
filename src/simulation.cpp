...
void run_simulation(const SimParams& p) {
    ...
    std::ofstream f_pos("output/data/positions.txt");
    std::ofstream f_vel("output/data/velocities.txt");
    std::ofstream f_ene("output/data/energies.txt");
    ...
    std::cout << "Saved to output/data/" << std::endl;
}
