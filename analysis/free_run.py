...
data = np.loadtxt("output/data/positions.txt")
times = data[:, 0]
N = (data.shape[1] - 1) // 3
...
np.savetxt("output/data/unlimited_positions.txt", unlimited_data)
print(f"Saved output/data/unlimited_positions.txt ({unlimited_data.shape[0]} rows, {unlimited_data.shape[1]} cols)")
...
vel_data = np.loadtxt("output/data/velocities.txt")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("output/plots/mean_free_path.png", dpi=150)
